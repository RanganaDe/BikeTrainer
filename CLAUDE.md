...
| `js/route.js` | Geocoding (Nominatim), routing (OSRM `router.project-osrm.org`, bike profile), street-name timeline from OSRM steps, POI fetch (Overpass), road elevation (Open-Meteo), and route progress. Holds `routeCoords/routeActive/routeTotalKm` etc. |
| `js/route-region.js` | Pure region detection (Switzerland/Netherlands/generic bboxes + elevation corroboration) and biome "region profiles" that `ride3d.js` reads to style terrain/scenery. |
